...
if v ~=nil and v.components.health and not v.components.health:IsDead() and v.components.combat and (v.components.combat.target == self.inst or v:HasTag("monster") or v:HasTag("werepig") or v:HasTag("frog")) and not v:HasTag("player") and not v:HasTag("companion") and not v:HasTag("stalkerminion") and not v:HasTag("smashable") and not v:HasTag("alignwall") and not v:HasTag("shadowminion") then 
...
function Mypower_musha:Lightning(victim)
	if victim and self.inst:HasTag("musha") and (victim.components.health and not victim.components.health:IsDead()) and self.inst.components.spellpower and self.inst.components.spellpower.current >10 then
	...
	self.inst.components.spellpower:DoDelta(-10)
