...
if not (inst.tiny_sleep or inst.sleep_on) and inst.components.fatigue_sleep.current >1 and not inst.sg:HasStateTag("walking") and not inst.sg:HasStateTag("running") and not inst.sg:HasStateTag("moving") and not inst.sg:HasStateTag("attack") and not inst.sg:HasStateTag("chopping") and not inst.sg:HasStateTag("mining") and not  inst.sg:HasStateTag("hammering") and not inst.sg:HasStateTag("digging") and not inst.sg:HasStateTag("netting") and not inst.sg:HasStateTag("fishing") and not inst:HasTag("playerghost") then
	if inst.dtired_veasy then
	inst.components.fatigue_sleep:DoDelta(-0.2)	
	elseif inst.dtired_easy then
	inst.components.fatigue_sleep:DoDelta(-0.1)
...
elseif inst.components.fatigue_sleep.current >=100 and not inst.sleep_on and not inst.tiny_sleep then
	if inst.dtired_veasy then
...
--old 
--[[
if inst.sleep_on and inst.components.stamina_sleep.current <= 75 and not inst.charging_music then
inst.charging_music = true
end
if inst.charging_music and inst.components.stamina_sleep.current >=100 then
...
if (inst.strength == "valkyrie" or inst.active_valkyrie) and not inst.sleep_on and not inst.tiny_sleep then --and inst.components.fatigue_sleep.current <= 100
 	if inst.dtired_veasy then
	inst.components.fatigue_sleep:DoDelta(0.1)	
	elseif inst.dtired_easy then
	inst.components.fatigue_sleep:DoDelta(0.2)
...
if (inst.strength == "berserk" or inst.berserk) and not inst.sleep_on and not inst.tiny_sleep then --and inst.components.fatigue_sleep.current <= 100
 	if inst.dtired_veasy then
	inst.components.fatigue_sleep:DoDelta(0.1)	
	elseif inst.dtired_easy then
	inst.components.fatigue_sleep:DoDelta(0.2)
...
local function on_Lshield_1(inst, attacked)
   local Lshield1 = 0.2
   if math.random() < Lshield1 and inst.components.sanity.current >= 1 and not inst.components.health:IsDead() and inst.components.spellpower:GetPercent() >0 then
...
local function on_Lshield_2(inst, attacked)
   local Lshield2 = 0.24
   if math.random() < Lshield2 and inst.components.sanity.current >= 1 and not inst.components.health:IsDead() and inst.components.spellpower:GetPercent() >0 then
	SpawnPrefab("shock_fx").Transform:SetPosition(inst:GetPosition():Get())
	inst.SoundEmitter:PlaySound("dontstarve/common/lightningrod")
...
local function on_Lshield_3(inst, attacked)
   local Lshield3 = 0.28
   if math.random() < Lshield3 and inst.components.sanity.current >= 1 and not inst.components.health:IsDead() and inst.components.spellpower:GetPercent() >0 then
	SpawnPrefab("shock_fx").Transform:SetPosition(inst:GetPosition():Get())
	inst.SoundEmitter:PlaySound("dontstarve/common/lightningrod")
...
local function on_Lshield_4(inst, attacked)
   local Lshield4 = 0.32
   if math.random() < Lshield4 and inst.components.sanity.current >= 1 and not inst.components.health:IsDead() and inst.components.spellpower:GetPercent() >0 then
	SpawnPrefab("shock_fx").Transform:SetPosition(inst:GetPosition():Get())
	inst.SoundEmitter:PlaySound("dontstarve/common/lightningrod")
...
local function on_Lshield_5(inst, attacked)
   local Lshield5 = 0.36
   if math.random() < Lshield5 and inst.components.sanity.current >= 1 and not inst.components.health:IsDead() and inst.components.spellpower:GetPercent() >0 then
	SpawnPrefab("shock_fx").Transform:SetPosition(inst:GetPosition():Get())
	inst.SoundEmitter:PlaySound("dontstarve/common/lightningrod")
...
if inst.components.stamina_sleep.current < 35 and inst.components.rider:IsRiding() then
...
elseif inst.components.stamina_sleep.current < 35 and inst.components.stamina_sleep.current >= 10 and not inst.components.rider:IsRiding() then
	if inst:HasTag("groggy") and sleep_walker and not idle then
		inst:RemoveTag("groggy")
		inst.components.locomotor.walkspeed = (TUNING.WILSON_WALK_SPEED * 0.85 )	
		inst.components.locomotor.runspeed = (TUNING.WILSON_RUN_SPEED * 0.85) 
