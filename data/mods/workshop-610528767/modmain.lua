...
if LANGUAGE == "detect" then --We should try to detect the language --我们应尝试去检测语言
	if HAS_MOD.BRAZIL then
		LANGUAGE = "brazil"
	elseif HAS_MOD.CHINESE then
		LANGUAGE = "chinese" --we use this for Chinese --我们用这个给中国人
	else
		LANGUAGE = "english" --we use this for English --我们用这个来学英语
	end
end
