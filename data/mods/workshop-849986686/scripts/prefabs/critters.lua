...
local function oneat(inst, food)
    ...
    if inst.prefab == "critter_puppy" then --小狗
        if not owner:HasTag("puppypower") then
            print("puppy power!")
            owner:AddTag("puppypower")
            if owner.components.combat.damagemultiplier then
                owner.components.combat.damagemultiplier = owner.components.combat.damagemultiplier*BUFFMULT
            else owner.components.combat.damagemultiplier = BUFFMULT end
            owner:DoTaskInTime(BUFFTIME, function()
                owner.components.combat.damagemultiplier = owner.components.combat.damagemultiplier/BUFFMULT
                owner:RemoveTag("puppypower")
            end)
        end
    end
    if inst.prefab == "critter_kitten" then --小猫
        if not owner:HasTag("kittenspeed") then
            print("kitten speed!")
            owner:AddTag("kittenspeed")
            owner.components.locomotor.walkspeed = owner.components.locomotor.walkspeed*BUFFMULT
            owner.components.locomotor.runspeed = owner.components.locomotor.runspeed*BUFFMULT
            owner:DoTaskInTime(BUFFTIME, function()
                owner.components.locomotor.walkspeed = owner.components.locomotor.walkspeed/BUFFMULT
                owner.components.locomotor.runspeed = owner.components.locomotor.runspeed/BUFFMULT
...
local function MakeCritter(name, animname, face, diet, flying, data)
    ...
    local function fn()
        ...
        if name == "critter_dragonling" then --小蜻蜓
        ...
        if name == "critter_glomling" then --小格格姆
